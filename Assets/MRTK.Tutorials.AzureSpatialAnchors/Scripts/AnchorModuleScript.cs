...
public class AnchorModuleScript : MonoBehaviour
{
    ...
    {
        Debug.Log("\n AZURE:  AnchorModuleScript.CreateAzureAnchor()");
        ...
        {
            Debug.Log("AZURE: Didn't get the local anchor...");
            return;
        }
        else
        {
            Debug.Log("AZURE: Local anchor created");
        }
        ...
        while (!cloudManager.IsReadyForCreate)
        {
            ...
            QueueOnUpdate(new Action(() => Debug.Log($"AZURE: Please move to capture more environment: {createProgress:0%}")));
        }
        ...
        try
        {
            Debug.Log("AZURE: Preparing Azure anchor... please wait...");
            ...
            if (success)
            {
                Debug.Log($"AZURE: Azure anchor with ID = '{currentCloudAnchor.Identifier}', created successfully");

                // Update the current Azure anchor ID
                Debug.Log($"AZURE: Current Azure anchor ID updated to '{currentCloudAnchor.Identifier}'");
            ...
            {
                Debug.Log($"AZURE: Failed to save cloud anchor with ID '{currentAzureAnchorID}' to Azure");
